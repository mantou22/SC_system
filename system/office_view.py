...
class OfficeAdd(BaseView):
    ...
    def get(self,request):
        return render(request, 'system_office_form.html',{"method":"add"})
...
class OfficeGetChildren(BaseView):
    ...
    def get(self,request):
        ...
        if parentId=='-1':
            parentId = 0
        o_list = office.objects.filter(parent_id=parentId,is_delete=0).annotate(parentId=F('parent_id')).values('parentId','id','name','sort','type','code','useable')
        data = list(o_list)
        return JsonResponse(data,safe=False)
